...
namespace AudioBooks.AudioBooksData
{
    #region SqlHelper
        ...
        public static string GetConnectionString(string connectionString = null)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(connectionString))
                    connectionString = ConfigurationManager.AppSettings["conACM"];

                return config.ConnectionStrings.ConnectionStrings[connectionString].ToString();
            }
            catch (NullReferenceException nex)
            {
                throw new Exception($"The ConnectionString for '{connectionString}' was not setup correctly");
            }
        }
